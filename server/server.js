...
if (port == null || port == "") { //set up for deployment OR dev
  port = 3000;
}
//eslint is messing up here, FIX ME
...
const checkJwt = jwt({
  // used as middleware to protect certain routes, see james
  secret: jwksRsa.expressJwtSecret({
    cache: true,
    rateLimit: true,
    jwksRequestsPerMinute: 5,
    jwksUri: `https://FILL_ME_IN/.well-known/jwks.json`
  }),
  // Validate the audience and the issuer.
  audience: "https://FILLMEIN/userinfo",
  issuer: `https://FILL_ME_IN/`,
  algorithms: ["RS256"]
});
...
//anything else(especially with auth0) redirects to the home page (not protected)
app.get("/*", (err, res) => {
...
const getLiveCoinData = () => { //
  ...
  for (let i = 0; i < coinNames.length; i++) {
    let dummyPromise = axios
      ...
      }).catch(() => console.log('error getting', coinNames[i])); //edited late and not tested
...
setInterval(() => { //this is why you need a second api key, this makes the full 5 calls right away, which is why it waits 70 seconds before calling again
  console.log('tick');
  getLiveCoinData();
}, 70000)